...
class Tcp:
    """
    Software-emulated transfers over TCP, for development and tests.
    ...
    Attributes:
        bind (Optional[str]): Source address to bind to. TCP binds an IP address
            where ``EfaDirect`` has none, so this has no counterpart there. If
...
# The fabric provider carrying RDMA transfers.
RdmaProvider = Union[EfaDirect, Tcp]


@dataclass
class RdmaConfiguration:
    """
    Represents the configuration for remote direct memory access (RDMA) transfers.
    ...
    no fabric is opened and libfabric is never linked.

    A client configured for RDMA cannot also use compression, ``lazy_connect``,
    read-only mode, or static node discovery. GLIDE refuses those combinations
    when the client is created.

    Attributes:
        provider (RdmaProvider): The fabric provider to open, ``EfaDirect()`` or
            ``Tcp()``.
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.provider, (EfaDirect, Tcp)):
            raise ConfigurationError(
                "provider must be EfaDirect() or Tcp(), got "
                f"{type(self.provider).__name__}"
            )

        if isinstance(self.provider, Tcp) and self.provider.bind is not None:
    ...
    def _to_protobuf(self) -> ProtobufRdmaConfig:
        ...
        if isinstance(self.provider, Tcp):
...
class BaseClientConfiguration:
    ...
    Attributes:
        ...
        rdma (Optional[RdmaConfiguration]): Configuration for RDMA direct memory access transfers.
            ⚠️ WARNING: This feature is experimental and not recommended for production use.
            When set, the client opens a fabric endpoint and handshakes each node as it connects to it,
            so ``rdma_get`` and ``rdma_set`` can move values directly between server memory and memory
            the caller registered. Only the synchronous client compiled with the RDMA feature supports
            it. Cannot be combined with compression, ``lazy_connect``, read-only mode, or static node
            discovery. Transfers always run against the primary, so a ``ReadFrom`` that prefers a
            replica does not apply to them. If not set, RDMA is disabled.
