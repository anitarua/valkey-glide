...
def test_efa_direct_sets_the_provider_oneof(cluster):
    request = _request(RdmaConfiguration(provider=EfaDirect()), cluster)
...
def test_tcp_sets_the_provider_oneof(cluster):
    ...
    request = _request(RdmaConfiguration(provider=Tcp()), cluster)
...
def test_interface_and_bind_reach_the_request():
    request = _request(
        RdmaConfiguration(provider=Tcp(bind="127.0.0.1"), interface="eth0")
    )
...
def test_interface_is_optional():
    request = _request(RdmaConfiguration(provider=EfaDirect()))
...
def test_efa_direct_has_no_bind_to_set():
    ...
    with pytest.raises(TypeError):
        EfaDirect(bind="127.0.0.1")  # type: ignore[call-arg]


def test_a_provider_is_required_to_be_a_provider():
    with pytest.raises(
        ConfigurationError, match="must be EfaDirect\\(\\) or Tcp\\(\\)"
    ):
...
@pytest.mark.parametrize(
    ("config", "field"),
    [
        (lambda: RdmaConfiguration(provider=Tcp(), interface=""), "interface"),
        (lambda: RdmaConfiguration(provider=Tcp(bind="")), "bind"),
    ],
    ids=["interface", "bind"],
)
...
def test_reassigned_fields_are_revalidated_when_the_request_is_built():
    ...
    config = RdmaConfiguration(provider=Tcp())
    config.provider = "tcp"  # type: ignore[assignment]

    with pytest.raises(ConfigurationError, match="must be EfaDirect"):
        config._to_protobuf()
...
def test_the_async_client_refuses_an_rdma_configuration():
    ...
    config = GlideClientConfiguration(
        addresses=ADDRESSES, rdma=RdmaConfiguration(provider=Tcp())
    )
