...
The transfer tests need a server that serves the ``LO.*`` commands, which this
repository cannot start, so they are skipped unless ``GLIDE_RDMA_SERVER`` names
one. Everything else runs anywhere, including on a build with no RDMA compiled
in: the surface is present either way and answers with a reason.
"""
...
def test_registering_without_a_fabric_names_the_cause(glide_sync_client: GlideClient):
    ...
    # it. The error has to say which configuration is missing.
    with pytest.raises(RdmaError, match="RdmaConfiguration"):
...
def test_an_rdma_client_against_a_plain_server_still_works(
    ...
    config = type(glide_sync_client._config)(
        ...
        rdma=RdmaConfiguration(provider=Tcp()),
    )
...
class TestRdmaTransfers:
    ...
    def rdma_client(self):
        ...
        provider = EfaDirect() if named == "efa-direct" else Tcp()
