...
ENCODING = "utf-8"
# How the core begins the message of a transfer cancelled by closing the client.
...
class RdmaReadReceipt:
    ...
    Attributes:
        ...
        checksum (Optional[int]): CRC-32c the server computed over those bytes.
            It arrives unasked, from a server new enough to send it, and the
            landed bytes have already been verified against it.
    """
...
class RdmaWindow:
    ...
    Attributes:
        ...
        length (int): How many bytes it covers. On a read this is the room the
            value may use; on a write it is how much to send.
    """
...
class RdmaRegion:
    """
    Memory registered with the server so it can be transferred into or out of
    directly.

    The memory is the caller's: this holds a reference to it and registers it
    with the client's fabric, but never copies or owns it. Registration pins the
    pages, which is expensive and bounded by ``RLIMIT_MEMLOCK``, so register one
    ...
    Create one with :meth:`BaseClient.register_rdma_region`, not directly. Can be
    ...
    def memoryview(self) -> memoryview:
        """
        A view of the registered memory, for reading what a transfer landed or
        staging what one will send.

        Always a view of bytes, whatever the buffer underneath is made of. A
        window is a span of bytes, because that is what was registered and what
        the server addresses, so a view that indexed by element would put the
        caller's staging area somewhere other than where the transfer runs for
        any buffer whose items are wider than a byte, such as a NumPy array of
        integers.
    ...
    def close(self) -> None:
        ...
        The caller's buffer is not freed: it is theirs to reuse once this
        returns. Closing twice is a no-op, so this is safe in a ``finally``.

        Any transfer against the region must have finished first. This is not a
        way to cancel one: the server may still be moving bytes into these
        pages, and unpinning them underneath it is undefined for both ends. To
        cancel a transfer in flight, close the client from another thread, then
        close the region once the transfer call has returned.
        """
...
class BaseClient(CoreCommands):
    ...
    def rdma_available() -> bool:
        ...
        The published packages do, so this is normally True. A build made from
        source without the RDMA feature does not.

        This says nothing about the machine. For whether a transfer can actually
        happen here, use :meth:`rdma_usable`.
    ...
    def rdma_usable() -> bool:
        ...
        A ``False`` carries no detail. To find out why, pass the configuration.
    ...
    def register_rdma_region(self, memory: Any) -> RdmaRegion:
        ...
        Returns:
            RdmaRegion: The registered region, to close when done with it.
    ...
    def rdma_get(
        ...
        Blocks until the transfer completes. The bytes are in the window by the
        time this returns. The value never travels in the reply, which carries
        only a byte count.
        ...
        registered, and only then lets this call return, raising
        ``ClosingError``, whether or not the server ever replies. Bytes that
        landed before the close stay, so the window then holds an unknown mix of
        old and new bytes. The region cannot be used for another transfer;
        close it once this call has returned.
    ...
    def rdma_set(
        ...
        Blocks until the transfer completes. The value is read from the window
        where it already lies, with no staging copy.

        As with ``rdma_get``, there is no timeout, and closing the client from
        another thread cancels the call, raising ``ClosingError``. A cancelled
        write stores either none of the new value or, if the server had just
        finished reading it, all of it; never part of it.
    ...
    def _rdma_window(self, window: RdmaWindow) -> Tuple[Any, int, int]:
        ...
        The bounds were checked when the window was made; what is checked here
        is that its region is still open and belongs to this client, which can
        stop being true between making a window and transferring with it.
        """
    ...
    def _handle_rdma_result(self, result) -> Optional[RdmaReadReceipt]:
        ...
        None means the key was absent, which is an outcome rather than an error:
        nothing was transferred and the window is untouched.

        A failure the core classified is raised as the class that classification
        names, so a transfer that timed out or lost its connection can be caught
        the same way as any other command that did. A transfer cancelled by
        closing the client raises ``ClosingError``, as any other call on a
        closed client does. Any other failure keeps its own class even if the
        client was closed meanwhile, so an overrun or a checksum mismatch that
        happened just before the close is still reported. Everything else raises
        ``RdmaError``, which those classes are also a kind of.
        """
    ...
    def close(self) -> None:
        ...
        The regions stay open until closed themselves, which is safe once the
        transfers using them have returned.
        """
