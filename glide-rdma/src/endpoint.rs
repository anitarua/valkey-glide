...
fn query_info_with(
    ...
    unsafe {
        ...
        if let Some(threading) = threading {
            // Asking matters as well as checking: a provider picks its locking to
            // suit the model it was asked for, so requesting one is how it is told
            // to be safe for concurrent use, not merely how it is interrogated.
...
///
/// Such a pool is typically shared - one allocation registered on every endpoint - so
/// granting remote write would let one server's stray write corrupt what every
/// endpoint is serving.
...
/// still live. That makes the `endpoint` <-> `fabric` dependency cyclic on purpose:
/// the cycle *is* the drop-ordering mechanism.
///
...
/// fails at the provider instead of landing. libfabric leaves this behaviour to each
/// provider rather than promising it; it was measured on efa-direct, where a close in
/// the middle of a transfer returned 0, the server's write failed, and no byte landed
/// after the close returned.
#[derive(Debug)]
...
struct RegistrationState {
    /// The open region, or `None` once it has been closed. The lock makes closing
    /// happen exactly once, and makes a second caller wait until the first close
    /// has finished rather than return while it is still under way.
...
impl Drop for Registration {
    ...
    ///
    /// Done here, not left to the last reference: a [`RevokeHandle`] that is revoking
    /// at this moment briefly holds one, and the memory must not be freed while the
    /// region is still open. If a revoke is under way, this waits for it to finish.
    fn drop(&mut self) {
        // Nothing can be done with a failure while dropping; the domain's own close
        // reclaims the region.
...
/// opening one is local bring-up that contacts no server. The only thing resembling a
/// connection is the separate RESP socket.
#[derive(Debug)]
...
#[cfg(test)]
mod tests {
    use super::{LibfabricEndpoint, check, query_info, query_info_requiring, threading_name};
    use crate::config::{FabricConfig, Provider};
    use crate::error::RdmaError;
    use ofi_libfabric_sys::bindgen::{
        fi_freeinfo, fi_threading_FI_THREAD_DOMAIN, fi_threading_FI_THREAD_FID,
        fi_threading_FI_THREAD_SAFE,
    };

    #[test]
    fn a_zero_return_code_is_success() {
        assert!(check(0, "fi_getinfo").is_ok());
    }
    ...
    #[test]
    fn every_threading_model_has_a_name() {
        assert_eq!(
            threading_name(fi_threading_FI_THREAD_SAFE),
            "FI_THREAD_SAFE"
        );
        assert_eq!(
            threading_name(fi_threading_FI_THREAD_DOMAIN),
            "FI_THREAD_DOMAIN"
        );
        assert_eq!(threading_name(u32::MAX), "an unrecognised");
    }

    // ---- endpoint lifecycle, against the real tcp provider ----

    fn tcp_endpoint() -> LibfabricEndpoint {
        LibfabricEndpoint::open(&FabricConfig::new(Provider::Tcp))
            .expect("the tcp provider should open on any host libfabric builds on")
    }

    #[test]
    fn opens_and_closes_a_tcp_endpoint() {
        let endpoint = tcp_endpoint();
        assert!(!endpoint.completion_queue().is_null());
        let address = tcp_endpoint()
            .local_address()
            .expect("an enabled endpoint has an address");
    ...
    #[test]
    fn registers_a_buffer_and_yields_a_remote_key() {
        let mut endpoint = tcp_endpoint();
        let buffer = vec![0u8; 4096];
        // SAFETY: `buffer` outlives the region, which is closed below.
        let region = unsafe { endpoint.register_remote(&buffer) }.expect("registration failed");
        assert!(!region.is_null());
        // SAFETY: closing a region this test opened, before its buffer drops.
        unsafe { ofi_libfabric_sys::bindgen::fi_close(&mut (*region).fid) };
    }

    #[test]
    fn registers_a_read_only_source_region() {
        let mut endpoint = tcp_endpoint();
        let buffer = vec![0u8; 4096];
        let region = unsafe { endpoint.register_source(&buffer) }.expect("registration failed");
        assert!(!region.is_null());
        unsafe { ofi_libfabric_sys::bindgen::fi_close(&mut (*region).fid) };
    }
}
