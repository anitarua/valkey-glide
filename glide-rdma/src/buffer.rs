...
impl RdmaBuffer {
    ...
    /// Deregister this buffer now, ahead of drop, and wake every transfer waiting on
    /// [`Self::revoked`]. A no-op if it is already revoked.
    ///
...
mod tests {
    ...
    /// The second revoke, and the drop after them, must find the region already
    /// closed rather than close it again.
    #[test]
    ...
    /// A transfer awaits `revoked()` while another thread closes the client.
    #[tokio::test]
    ...
    #[test]
    fn shared_memory_can_be_revoked() {
        let buffer = fabric().register_shared(Arc::new(vec![1u8; 128])).unwrap();
        buffer
            .revoke()
            .expect("a read-only registration closes the same way");
        assert!(buffer.is_revoked());
    }
