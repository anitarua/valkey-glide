...
///
/// Taken from the headers bindgen actually read, rather than written down here.
/// Those headers are whichever ones were on the build machine, so a fixed number
/// would be a guess that goes stale the moment someone builds against a different
/// libfabric — and it would be a guess about the one thing this constant exists
/// to be exact about.
...
/// keeps it correct for whatever target is being built, which a written-out
/// number would not be.
...
mod tests {
    use super::{
        FI_ENODATA, HEADER_API_VERSION, SYMBOLS, ensure_loaded, header_api_version, resolved,
    };

    #[test]
    fn libfabric_loads_on_a_host_that_has_it() {
        // The whole point of the module: no hard link, and it still finds the
        // library. If this fails the rest of the suite cannot run either.
        ensure_loaded().expect("libfabric should load on a host with it installed");
    }

    #[test]
    fn every_bootstrap_symbol_resolves() {
        ensure_loaded().expect("libfabric should load");
        let table = resolved().expect("a successful load leaves a table");
        for (address, name) in table.address.iter().zip(SYMBOLS) {
            assert!(
                !address.is_null(),
                "{} resolved to nothing",
                name.to_string_lossy()
    ...
    #[test]
    fn the_header_version_is_the_one_the_bindings_came_from() {
        // Compared against the generated constants rather than a number written
        // here, because the answer legitimately differs between build machines.
        // What must hold is that the two agree.
        assert_eq!(
            header_api_version() >> 16,
            ofi_libfabric_sys::bindgen::FI_MAJOR_VERSION,
            "major version"
        );
        assert_eq!(
            header_api_version() & 0xffff,
        ...
        // libfabric has been on 2.x since before this crate existed; a 0 here
        // would mean the constants did not come through at all.
        assert!(header_api_version() >> 16 >= 2, "major version looks unset");
    }

    #[test]
    fn the_loaded_library_is_at_least_as_new_as_the_headers() {
        ensure_loaded().expect("libfabric should load");
        // SAFETY: fi_version is one of the resolved symbols and takes no arguments.
        let runtime = super::fi_version();
        assert!(
            runtime >= HEADER_API_VERSION,
            "loaded libfabric reports {}.{}, older than the headers' {}.{}",
            runtime >> 16,
            runtime & 0xffff,
            HEADER_API_VERSION >> 16,
            HEADER_API_VERSION & 0xffff,
