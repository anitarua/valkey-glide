...
///
/// Opaque on purpose: it is a libfabric `fi_addr_t`, meaningful only to the address
/// vector that produced it. Entries are minted and released only by the peer
/// refcount, so that an address stays in the vector for exactly as long as some
/// session still needs the server to be reachable at it.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
struct PeerHandle(u64);

/// A local domain. Every [`RdmaBuffer`] holds a clone, keeping
