...
//! the the RDMA transfer should be performed.
...
pub const HELLO: &str = "LO.HELLO";

/// The command that reads a stored object into registered memory.
pub const GET: &str = "LO.GET";

/// The command that stores an object read out of registered memory.
pub const SET: &str = "LO.SET";

/// Whether a command by this name can only run on a connection that already has
/// an open session.
///
/// The transfers can, because the server looks up where to reach this client's
/// memory by the connection the command arrived on. `LO.HELLO` is what
/// establishes that, so it is deliberately not one of these.
...
///
/// The server writes the whole stored object to the named address. Note what is
/// absent: the client cannot bound the transfer, so an object larger than the
/// window the caller had in mind overruns whatever follows it in the same
/// registration. Only the registration's own end is enforced, by the fabric.
///
// TODO(lo-get-length): send the length as a fourth argument once module accepts it.
