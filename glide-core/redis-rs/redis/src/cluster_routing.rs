...
mod tests_routing {
    ...
    #[test]
    fn test_routing_info_rdma_ignores_the_memory_arguments() {
        // The arguments after the key are a local memory registration, and two
        // transfers of the same key from different buffers must still meet at the
        // same shard.
        for name in ["LO.GET", "LO.SET"] {
            let mut elsewhere = rdma_cmd(name, "foo");
            elsewhere.arg("8675309");

            assert_eq!(
                slot_of(&elsewhere),
                slot_of(&rdma_cmd(name, "foo")),
                "{name} routing must depend on nothing but the key"
            );
        }
    }

    #[test]
    fn test_routing_info_rdma_different_keys_use_their_own_slots() {
        assert_ne!(slot_of(&rdma_get("foo")), slot_of(&rdma_get("bar")));
    }
