...
//! Raw libfabric calls: querying what the host offers and turning return codes into [`DmaError`]s.
...
pub(crate) fn check(code: i32, operation: &'static str) -> Result<(), DmaError> {
    if code == 0 {
        return Ok(());
    }
...
fn validate(config: &FabricConfig) -> Result<(), DmaError> {
...
///
/// Holds a [`DmaFabric`] so the domain cannot close while a registration against it is
...
// SAFETY: a `Registration`'s only operation after construction is `fi_close` in `Drop`,
// which synchronises with the domain like `fi_mr_reg` did. Deregistration cannot run
// concurrently with any other call into the domain.
...
impl Drop for Registration {
    fn drop(&mut self) {
        ...
        // SAFETY: this registration owns the region, live until this drop closes it.
...
// SAFETY: libfabric permits its objects to move between threads so long as they are
// not used concurrently. Callers keep the endpoint behind a lock, so every call
// through these handles is exclusive.
...
impl LibfabricEndpoint {
    ...
    /// Register host memory the server may only *read*, for the `DMA.SET` direction.
    ///
    /// # Safety
    /// `buffer` must stay allocated and unmoved until the region is closed.
    ...
    /// # Safety
    /// `buffer` must stay allocated and unmoved until the region is closed.
    unsafe fn register(&mut self, buffer: &[u8], access: u64) -> Result<*mut fid_mr, DmaError> {
    ...
    /// every address `DMA.HELLO` returned must be inserted before the first transfer.
...
mod tests {
    ...
    fn a_failure_carries_its_errno_and_libfabric_message() {
        // -61 is -FI_ENODATA on Linux and macOS alike.
    ...
    fn bind_is_rejected_on_efa_direct() {
        ...
        assert!(
            matches!(&error, DmaError::Configuration(message) if message.contains("efa-direct")),
    ...
    fn an_unknown_interface_reports_what_exists() {
        ...
        let DmaError::Configuration(message) = &error else {
