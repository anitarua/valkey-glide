...
#[cfg(test)]
mod tests {
    use super::{FabricConfig, Provider};

    #[test]
    fn the_default_provider_is_the_hardware_one() {
        assert_eq!(Provider::default(), Provider::EfaDirect);
        assert_eq!(FabricConfig::default().provider(), Provider::EfaDirect);
    }

    #[test]
    fn the_provider_is_whatever_was_named() {
    ...
    fn options_default_to_unset() {
        let config = FabricConfig::new(Provider::EfaDirect);
        assert_eq!(config.interface(), None);
        assert_eq!(config.bind(), None);
    }

    #[test]
    fn builders_record_what_they_are_given() {
        let config = FabricConfig::new(Provider::Tcp)
            .with_interface("rdmap16s27")
            .with_bind("127.0.0.1");
        assert_eq!(config.interface(), Some("rdmap16s27"));
        assert_eq!(config.bind(), Some("127.0.0.1"));
    }
}
