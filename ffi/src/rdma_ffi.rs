...
// and RESP responses contain only a byte count and an optional checksum. The server
// reads/writes the registered region directly.
// ═══════════════════════════════════════════════════════════════════════════════

use super::*;

/// Whether this build has RDMA compiled in.
///
/// Published artifacts do. A build made from source without the `rdma` feature
/// does not, and then every function below refuses.
///
/// This says nothing about the machine. The parts of the RDMA surface that do
/// not touch a fabric — [`rdma_checksum`] is the one — work wherever this is
/// true. Configuring a client for RDMA needs more: see [`rdma_usable`].
#[unsafe(no_mangle)]
...
///
/// [`rdma_available`] and libfabric both. libfabric is looked for the first time
/// this is called — in the places EFA installs it, then wherever the system
/// keeps libraries — and the answer is remembered for the life of the process,
/// so this is cheap to call repeatedly and cannot change under a caller.
///
/// This is the one to check before passing an RDMA configuration to a client. It
/// is deliberately separate from [`rdma_available`] because the two questions
/// have different answers on the same machine: a published build on a laptop
/// with no libfabric has RDMA compiled in and cannot open a fabric.
///
/// A `false` carries no detail. To find out why, configure a client for RDMA
/// anyway: that path fails with a message naming either the missing support or
/// every path it searched.
#[unsafe(no_mangle)]
...
/// A registered region of caller-owned memory, opaque across the boundary.
///
...
/// A registered region of caller-owned memory, opaque across the boundary.
/// Unused without the `rdma` feature.
#[cfg(not(feature = "rdma"))]
pub enum RdmaRegion {}
...
impl RdmaResult {
    ...
    /// A caller mistake rather than a transfer failure: a null pointer, a zero
    /// length, a client that cannot serve RDMA.
...
///
/// A message carrying an interior NUL cannot be a C string, so it is replaced
/// rather than dropped: a caller learning nothing is worse than a caller
/// learning the message was unrepresentable.
...
/// CRC-32c of `length` bytes at `data`, the integrity check the RDMA protocol
/// carries.
///
/// # Returns
///
...
/// Register caller-owned memory so the server can transfer into or out of it.
///
/// # Returns
///
    ...
    #[cfg(feature = "rdma")]
    {
            ...
            .register_rdma_region(ForeignRegion { memory, length })
        {
            Ok(buffer) => RdmaRegistration {
                error_message: std::ptr::null(),
                region: Box::into_raw(Box::new(RdmaRegion { buffer })),
            },
...
/// registered region.
///
/// Blocks until the transfer completes, with no timeout. The server moves the
/// payload with a remote memory operation that nothing can call off once posted,
/// so returning early would leave the server writing into the window while the
/// caller treated it as free.
///
/// To cancel a transfer, call [`close_client`] from another thread. That revokes
/// every region the client registered, so the server can no longer reach them,
/// and only then lets this call return, with an error, whether or not the server
/// ever replies. Bytes that landed before the close stay, so the window then
/// holds an unknown mix of old and new bytes. The region cannot be used for
/// another transfer; free it with [`free_rdma_region`] once this call returns.
///
/// One region serves many transfers at different offsets without being
/// registered again.
///
/// `length` is the room the caller means to give the value, but it does not
/// bound the transfer: the command carries no length, so the server writes the
/// whole stored object, and a value larger than `length` overruns whatever
/// follows it in the same region. Only the region's own end is enforced, by the
/// fabric. The overrun is reported after the fact -- the call fails once the
/// reply says more bytes landed than `length` allowed -- so a caller packing
/// several windows into one region must size them for the largest value it can
/// store, or keep one region per transfer in flight.
///
/// When the server sends a CRC-32c with its reply, the client verifies the
/// landed bytes against it and fails the call on a mismatch. The server decides
/// whether to send one; the caller cannot ask for it.
///
/// # Returns
///
...
///
/// * `client_adapter_ptr` must be a pointer from [`create_client`] that
///   [`close_client`] has not been called on when this call starts. Once the
///   call is under way, calling [`close_client`] from another thread is allowed:
///   that is how a transfer is cancelled.
...
pub unsafe extern "C-unwind" fn rdma_get(
    ...
    #[cfg(feature = "rdma")]
    {
        ...
        let outcome = block_on_transfer(
            &adapter,
            client.rdma_get(key, &mut region.buffer, offset, length),
        );
...
///
/// Blocks until the transfer completes. The window must lie inside the region,
/// and `length` is exactly what the server reads, so a write cannot overrun the
/// way a read can. Like [`rdma_get`], it has no timeout, and calling
/// [`close_client`] from another thread cancels it. A cancelled write stores
/// either none of the new value or, if the server had just finished reading it,
/// all of it; never part of it.
///
/// No integrity check is sent: the command carries the length and the memory
/// reference and nothing else. A caller that wants one can compute
/// [`rdma_checksum`] over the same bytes and compare it against its own record.
///
/// # Returns
///
/// A [`RdmaResult`] the caller frees with [`free_rdma_result`].
...
pub unsafe extern "C-unwind" fn rdma_set(
    ...
    #[cfg(feature = "rdma")]
    {
        ...
        let outcome = block_on_transfer(
            &adapter,
            client.rdma_set(key, &region.buffer, offset, length),
        );
...
mod tests {
    ...
    #[test]
    fn availability_reports_what_was_compiled_in() {
        assert_eq!(rdma_available(), cfg!(feature = "rdma"));
    }

    #[test]
    fn usability_needs_the_feature_and_a_usable_libfabric() {
        // Without the feature the answer is always no, whatever the machine has.
        #[cfg(not(feature = "rdma"))]
        assert!(!rdma_usable(), "a build without the feature cannot do RDMA");

        // With the feature it reports the machine, so it must agree with the
        // thing that actually opens a fabric. Both outcomes are legitimate: a
        // builder without libfabric says no, a fabric host says yes.
        #[cfg(feature = "rdma")]
        assert_eq!(
            rdma_usable(),
            glide_rdma::ensure_loaded().is_ok(),
            "usability disagreed with whether libfabric could be loaded"
        );
    }

    #[test]
    fn usability_is_never_broader_than_availability() {
        // A machine cannot carry a transfer that was not built in. The reverse is
        // allowed and ordinary: compiled in, but no libfabric here.
        assert!(!rdma_usable() || rdma_available());
    }

    #[test]
    fn usability_is_stable_across_calls() {
        // The lookup happens once and is remembered, so a caller polling it does
        // not pay for it repeatedly and cannot see it change mid-process.
        assert_eq!(rdma_usable(), rdma_usable());
    }
    ...
    /// Argument checks come first, so a build without RDMA still rejects a bad
    /// call on its own terms rather than blaming the missing feature.
    #[cfg(not(feature = "rdma"))]
    #[test]
    ...
    /// A region registered against the software tcp provider, so a test can
    /// exercise the window arithmetic without fabric hardware or a server.
    #[cfg(feature = "rdma")]
    fn test_region(bytes: Vec<u8>) -> RdmaRegion {
        let fabric =
        ...
        RdmaRegion {
            buffer: fabric.register(bytes).expect("registers"),
        }
    }

    #[cfg(feature = "rdma")]
    #[test]
    fn a_region_reports_the_registered_size() {
        let region = test_region(vec![0u8; 4096]);
        assert_eq!(unsafe { rdma_region_capacity(&region) }, 4096);
    }
